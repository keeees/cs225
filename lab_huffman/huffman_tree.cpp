...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while(singleQueue.size()+mergeQueue.size() != 1){
         ...
         cout<<temp->freq.getCharacter()<<" first smallest"<<endl;

         if(singleQueue.size()+mergeQueue.size() != 0){
             temp2 = removeSmallest(singleQueue,mergeQueue);
            // cout<<temp2->freq.getCharacter()<<" second smallest"<<endl;

        }else{// if size is 0
             parent = new TreeNode(Frequency(temp->freq.getFrequency()+temp2->freq.getFrequency()));
             parent->left = temp;
             mergeQueue.push(parent);
             break;
         }
            //temp->freq.getCharacter()+temp2->freq.getCharacter());
         parent = new TreeNode(Frequency(temp->freq.getFrequency()+temp2->freq.getFrequency()));
         parent->left = temp;
         parent->right = temp2;
         mergeQueue.push(parent);
    ...
    while (bfile.hasBits()) {
         ...
         */
         ...
         if(current->left == NULL && current->right == NULL){
             ...
             std::cout <<current->freq.getCharacter() << '\n';
             current = root_;
         }
    }
}
