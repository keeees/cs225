...
class BinaryTree
{
  ...
  public:
    ...
    T getLeftMost(const Node* subRoot) const;
    T getRightMost(const Node* subRoot) const;

  private:
      void printPaths(vector<vector<T> > &paths,     vector<T> &temp, const Node* subRoot) const;
      int sumDistances(const Node* subRoot, int distance, int total) const;
