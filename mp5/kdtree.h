...
class KDTree
{
  ...
  public:
    ...
    bool smallerDimVal(const Point<Dim>& first, const Point<Dim>& second,
                       int curDim) const;
    int quickselect(int left,int right, int k, int dimension );
    ...
    void helper(KDTreeNode *& subroot, int start, int end ,int dimension);
    Point<Dim> findhelper(const Point<Dim>&query, int dimension,KDTreeNode *current,const Point<Dim>& best)const ;
    // functions used for grading:
    double radiusFinder(const Point<Dim>& query,const Point<Dim>& other) const;
    /**
